...
class LOAD_MNIST():
    ...
    def __init__(self, diretorio='cifar-10-batches-py', 
        ...
        ######### Carregando o dataset original do MNIST ############
        print('Carregando MNIST DATASET da pasta: ')

        (self.train_data, self.train_labels), (self.test_data, self.test_labels) = mnist.load_data()

        
        #self.train_data = self.train_data.transpose((0, 2, 3, 1))# convert to hwc ?
        self.train_data=np.array([np.dstack([i]*3) for i in self.train_data ] )

        self.test_data=np.array([np.dstack([i]*3) for i in self.test_data ] )
